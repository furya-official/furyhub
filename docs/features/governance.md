...
Any Fury holder, whether bonded or unbonded, can submit proposals by sending a
...
fury tx gov submit-proposal param-change proposal.json --from=<key-name> --fees=0.3fury --chain-id=furyhub
```
...
fury tx gov submit-proposal community-pool-spend proposal.json --from=<key-name> --fees=0.3fury --chain-id=furyhub
```
